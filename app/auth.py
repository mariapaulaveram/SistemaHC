...
def create_session_cookie(response, usuario_id: int) -> None:
    token = _get_serializer("session").dumps({"id": usuario_id})
    response.set_cookie(
        SESSION_COOKIE,
        token,
...
def get_current_user_id(request: Request) -> int | None:
    ...
    if not token:
        return None
    try:
        data = _get_serializer("session").loads(token, max_age=SESSION_MAX_AGE)
        return data.get("id")
